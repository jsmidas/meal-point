...
export default function ProductDetailPage() {
  ...
  const renderSection: Record<string, () => React.ReactNode> = {
      ...
      <section key="hero" className="relative pt-16">
        {page.hero_image ? (
          <div className="relative h-[60vh] min-h-[400px]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16">
              <div className="max-w-6xl mx-auto">
                ...
                {page.subtitle && <p className="text-lg text-gray-300 max-w-xl">{page.subtitle}</p>}
              </div>
            </div>
          </div>
        ) : (
          <div className="pt-24 pb-16 px-6">
            <div className="max-w-6xl mx-auto">
              <p className="text-sm tracking-widest text-primary font-medium mb-3 uppercase">{product.category}</p>
              <h1 className="text-4xl md:text-6xl font-black text-text-primary mb-4">{product.name}</h1>
              {page.subtitle && <p className="text-lg text-text-secondary max-w-xl">{page.subtitle}</p>}
            </div>
          </div>
        )}
        {/* Price (히어로 바로 아래) */}
    ...
    keypoints: () => keyPoints.length > 0 ? (
      <section key="keypoints" className="py-20 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl font-bold text-text-primary text-center mb-12">핵심 포인트</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {keyPoints.map((kp, idx) => {
              const IconComp = iconMap[kp.icon] || Star;
              return (
                <div key={idx} className="text-center p-6 rounded-2xl border border-border bg-bg-card hover:border-primary/30 transition-colors">
                  <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-4">
                    <IconComp size={24} className="text-primary" />
                  </div>
                  <h3 className="font-semibold text-text-primary mb-2">{kp.title}</h3>
                  <p className="text-sm text-text-secondary">{kp.description}</p>
                </div>
              );
            })}
          </div>
    ...
    specs: () => specs.length > 0 ? (
      <section key="specs" className="py-20 px-6 border-t border-border">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="rounded-2xl border border-border overflow-hidden">
            {specs.map((spec, idx) => (
              <div key={idx} className={`flex ${idx < specs.length - 1 ? "border-b border-border" : ""}`}>
    ...
    detail: () => (page.detail_description || detailImages.length > 0) ? (
      <section key="detail" className="py-20 px-6 border-t border-border">
        <div className="max-w-4xl mx-auto">
          {page.detail_description && (
            <div className="prose prose-invert max-w-none mb-12" dangerouslySetInnerHTML={{ __html: page.detail_description }} />
          )}
    ...
    process: () => processSteps.length > 0 ? (
      <section key="process" className="py-20 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto">
          {page.process_title && (
            <h2 className="text-2xl font-bold text-text-primary text-center mb-12">{page.process_title}</h2>
          )}
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {processSteps.map((step, idx) => (
              <div key={idx} className="rounded-2xl border border-border bg-bg-card p-6 hover:border-primary/30 transition-colors">
                <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm mb-4">{step.step}</div>
    ...
    gallery: () => galleryImages.length > 0 ? (
      <section key="gallery" className="py-20 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="relative rounded-2xl overflow-hidden mb-4 aspect-video">
            <img src={galleryImages[galleryIdx]} alt={`Gallery ${galleryIdx + 1}`} className="w-full h-full object-cover" />
            {galleryImages.length > 1 && (
              <>
                <button onClick={() => setGalleryIdx((prev) => prev === 0 ? galleryImages.length - 1 : prev - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 flex items-center justify-center text-white hover:bg-black/70 transition-colors">
                  <ChevronLeft size={20} />
                </button>
                <button onClick={() => setGalleryIdx((prev) => prev === galleryImages.length - 1 ? 0 : prev + 1)} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 flex items-center justify-center text-white hover:bg-black/70 transition-colors">
          ...
          {galleryImages.length > 1 && (
            <div className="flex gap-2 overflow-x-auto pb-2">
              {galleryImages.map((url, idx) => (
                <button key={idx} onClick={() => setGalleryIdx(idx)} className={`shrink-0 w-20 h-20 rounded-xl overflow-hidden border-2 transition-colors ${idx === galleryIdx ? "border-primary" : "border-border hover:border-text-muted"}`}>
    ...
    figma: () => figmaEmbeds.length > 0 ? (
      <section key="figma" className="py-20 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto space-y-12">
          {figmaEmbeds.map((figma, idx) => {
            ...
            return (
              <div key={idx}>
                {figma.title && <h2 className="text-2xl font-bold text-text-primary text-center mb-8">{figma.title}</h2>}
                <div className="rounded-2xl overflow-hidden border border-border aspect-[16/10]">
                  <iframe src={embedUrl} className="w-full h-full" allowFullScreen title={figma.title || `Design ${idx + 1}`} />
                </div>
              </div>
            );
          })}
  ...
  return (
    <div className="min-h-screen bg-bg-dark">
      ...
      {/* 섹션을 section_order 순서대로 렌더링 */}
      {sectionOrder.map((sectionId) => renderSection[sectionId]?.() ?? null)}
